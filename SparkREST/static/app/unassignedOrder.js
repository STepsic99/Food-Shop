...
<div v-if="showFilters">
  ...
  <label>SVE</label>
  <input v-model="status" type="radio" name="status" value="NETRAŽENE">
  <label>NETRAŽENE</label><br><br>  
  Tip restorana:
				...
				var date2Cmp=new Date(this.date2)
    				if(this.searchedName && !this.orders[i].restaurant.name.toLowerCase().includes(this.searchedName.toLowerCase())){
								...
								break;	
											...
											control=0;
											break;	
									}	else if(this.status==="NETRAŽENE" && this.orders[i].requestedByMe){
										this.orders.splice(i,1);
											control=0;
											break;
